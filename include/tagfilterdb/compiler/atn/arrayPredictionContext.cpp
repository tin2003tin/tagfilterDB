#include "arrayPredictionContext.hpp"
...
bool predictionContextEqual(const Ref<const PredictionContext> &lhs,
    ...
    // parent PredictionContext pointers can be null during full context mode
    // and the ctxs are in an ArrayPredictionContext.  If both are null, return
    // true if just one is null, return false. If both are non-null, do
    // comparison.
    if (lhs == nullptr)
        return rhs == nullptr;
    if (rhs == nullptr)
        return false;           // lhs!=null and rhs==null
    return (*lhs).equals(*rhs); // both nonnull
}
...
bool ArrayPredictionContext::equals(const PredictionContext &other) const {
    ...
    const auto &array =
        support::downCast<const ArrayPredictionContext &>(other);
