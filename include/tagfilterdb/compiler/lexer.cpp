...
std::unique_ptr<Token> Lexer::nextToken() {
    ...
    outerContinue:
        ...
        do {
            ...
            size_t ttype;
            try {
                ttype = getInterpreter<atn::LexerATNSimulator>()->match(_input,
                                                                        mode);
...
#if DEBUG_LEXER == 1
    std::cout << std::string("popMode back to ") << modeStack.back()
              << std::endl;
#endif
...
void Lexer::notifyListeners(const LexerNoViableAltException & /*e*/) {
    ++_syntaxErrors;
    std::string text =
        _input->getText(support::Range(tokenStartCharIndex, _input->index()));
