...
//
// An Env is an interface used by the tagfilterdb implementation to access
...
#ifndef STORAGE_TAGFILTERDB_INCLUDE_ENV_H_
#define STORAGE_TAGFILTERDB_INCLUDE_ENV_H_
...
// This workaround can be removed when tagfilterdb::Env::DeleteFile is removed.
#if defined(_WIN32)
...
#undef DeleteFile
#define TAGFILTERDB_DELETEFILE_UNDEFINED
...
class Env {
  public:
    ...
    //
    // The result of Default() belongs to tagfilterdb and must never be deleted.
    static Env *Default();

    // Create an object that sequentially reads the file with the specified
    // name. On success, stores a pointer to the new file in *result and returns
    // OK. On failure stores nullptr in *result and returns non-OK.  If the file
    ...
    // status.  Implementations should return a NotFound status when the file
    // does not exist.
    //
    ...
    // the tagfilterdb implementation) must be prepared to deal with
    ...
    // DEPRECATED: Modern Env implementations should override RemoveFile
    // instead.
    //
    // The default implementation calls RemoveFile, to support legacy Env user
    // code that calls this method on modern Env implementations. Modern Env
    // user code should call RemoveFile.
    //
    ...
    // code that calls this method on modern Env implementations. Modern Env
    // user code should call RemoveDir.
    //
    // A future release will remove this method.
...
class TAGFILTERDB_EXPORT SequentialFile {
  public:
...
class TAGFILTERDB_EXPORT RandomAccessFile {
  public:
...
class TAGFILTERDB_EXPORT WritableFile {
  public:
...
class TAGFILTERDB_EXPORT Logger {
  public:
    Logger() = default;
...
class TAGFILTERDB_EXPORT FileLock {
  public:
    FileLock() = default;
...
TAGFILTERDB_EXPORT Status WriteStringToFile(Env *env, const DataView &data,
                                            const std::string &fname);

// A utility routine: read contents of named file into *data
TAGFILTERDB_EXPORT Status ReadFileToString(Env *env, const std::string &fname,
                                           std::string *data);

// An implementation of Env that forwards all calls to another Env.
// May be useful to clients who wish to override just part of the
// functionality of another Env.
class TAGFILTERDB_EXPORT EnvWrapper : public Env {
  public:
...
#if defined(_WIN32) && defined(TAGFILTERDB_DELETEFILE_UNDEFINED)
#if defined(UNICODE)
#define DeleteFile DeleteFileW
#else
#define DeleteFile DeleteFileA
#endif // defined(UNICODE)
#endif // defined(_WIN32) && defined(TAGFILTERDB_DELETEFILE_UNDEFINED)

#endif // STORAGE_TAGFILTERDB_INCLUDE_ENV_H_
