...
std::pair<char32_t, size_t>
tagfilterdb::support::Utf8::decode(std::string_view input) {
    assert(!input.empty());
...
tagfilterdb::support::Utf8::strictDecode(std::string_view input) {
    std::u32string output;
    char32_t codePoint;
    size_t codeUnits;
    output.reserve(
        input
            .size()); // Worst case is each byte is a single Unicode code point.
    for (size_t index = 0; index < input.size(); index += codeUnits) {
        ...
        if (codePoint == Unicode::REPLACEMENT_CHARACTER && codeUnits == 1) {
            // Condition is only met when an illegal byte sequence is
            // encountered. See Utf8::decode.
            return std::nullopt;
        }
...
std::u32string
tagfilterdb::support::Utf8::lenientDecode(std::string_view input) {
    std::u32string output;
    char32_t codePoint;
    size_t codeUnits;
    output.reserve(
...
std::string &tagfilterdb::support::Utf8::encode(std::string *buffer,
                                                char32_t codePoint) {
...
tagfilterdb::support::Utf8::strictEncode(std::u32string_view input) {
    std::string output;
    output.reserve(
        input.size() *
...
std::string
tagfilterdb::support::Utf8::lenientEncode(std::u32string_view input) {
    std::string output;
    output.reserve(
        input.size() *
        4); // Worst case is each Unicode code point encodes to 4 bytes.
