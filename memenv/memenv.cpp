...
class FileState {
  public:
    ...
    Status Read(uint64_t offset, size_t n, DataView *result,
        ...
        *result = DataView(scratch, n);
        return Status::OK();
    }

    Status Append(const DataView &data) {
...
class WritableFileImpl : public WritableFile {
  public:
    ...
    Status Append(const DataView &data) override { return file_->Append(data); }
    ...
    Status GetChildren(const std::string &dir,
        ...
        for (const auto &kvp : file_map_) {
            ...
            if (filename.size() >= dir.size() + 1 &&
                filename[dir.size()] == '/' &&
                DataView(filename).starts_with(DataView(dir))) {
