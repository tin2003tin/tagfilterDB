#ifndef STORAGE_TAGFILTERDB_UTIL_BITSET_H
#define STORAGE_TAGFILTERDB_UTIL_BITSET_H
...
class Bitset {
  public:
    ...
    Bitset(size_t bits) : size_(bits) { data_ = new char[(bits + 7) / 8](); }

    void set(size_t index) { data_[index / 8] |= (1 << (index % 8)); }

    void clear(size_t index) { data_[index / 8] &= ~(1 << (index % 8)); }
