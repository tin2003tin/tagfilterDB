...
//
// Copyright (c) 2011 The tagfilterdb Authors. All rights reserved.
...
int TestCloseOnExecHelperMain(char *pid_arg) {
    ...
    if (errno != EBADF) {
        std::fprintf(stderr,
                     "Unexpected errno after calling dup2 on fd %d: %s\n", fd,
                     std::strerror(errno));
...
void GetOpenFileDescriptors(std::unordered_set<int> *open_fds) {
    ...
    for (int fd = 0; fd < max_fd; ++fd) {
        if (::dup2(fd, fd) != fd) {
            // When given the same file descriptor twice, dup2() returns -1 if
            // the file descriptor is closed, or the given file descriptor if it
            // is open.
            //
            // Double-check that dup2() is saying the fd is closed.
            ASSERT_EQ(EBADF, errno) << "dup2() should set errno to EBADF on "
...
void GetNewlyOpenedFileDescriptor(
    ...
    ASSERT_EQ(1, open_fds.size()) << "Expected exactly one newly opened file "
                                     "descriptor during test setup";
...
void CheckCloseOnExecDoesNotLeakFDs(
    ...
    char *child_argv[] = {GetArgvZero()->data(), switch_buffer,
                          fd_buffer.data(), nullptr};
...
TEST_F(EnvPosixTest, TestOpenOnRead) {
    ...
    std::string test_dir;
    ASSERT_TAGFILTERDB_OK(env_->GetTestDirectory(&test_dir));
    ...
    // open-on-read behavior of POSIX Env tagfilterdb::RandomAccessFile.
    const int kNumFiles = kReadOnlyFileLimit + kMMapLimit + 5;
    tagfilterdb::RandomAccessFile *files[kNumFiles] = {0};
    for (int i = 0; i < kNumFiles; i++) {
        ASSERT_TAGFILTERDB_OK(env_->NewRandomAccessFile(test_file, &files[i]));
    }
    char scratch;
    DataView read_result;
    for (int i = 0; i < kNumFiles; i++) {
        ASSERT_TAGFILTERDB_OK(files[i]->Read(i, 1, &read_result, &scratch));
        ASSERT_EQ(kFileData[i], read_result[0]);
    }
    for (int i = 0; i < kNumFiles; i++) {
        delete files[i];
    }
    ASSERT_TAGFILTERDB_OK(env_->RemoveFile(test_file));
}
...
TEST_F(EnvPosixTest, TestCloseOnExecSequentialFile) {
    ...
    ASSERT_TAGFILTERDB_OK(WriteStringToFile(env_, "0123456789", file_path));

    tagfilterdb::SequentialFile *file = nullptr;
    ASSERT_TAGFILTERDB_OK(env_->NewSequentialFile(file_path, &file));
    CheckCloseOnExecDoesNotLeakFDs(open_fds);
    delete file;

    ASSERT_TAGFILTERDB_OK(env_->RemoveFile(file_path));
}
...
TEST_F(EnvPosixTest, TestCloseOnExecRandomAccessFile) {
    ...
    // mmap region.
    tagfilterdb::RandomAccessFile *mmapped_files[kMMapLimit];
    for (int i = 0; i < kMMapLimit; i++) {
        ASSERT_TAGFILTERDB_OK(
            env_->NewRandomAccessFile(file_path, &mmapped_files[i]));
    }

    tagfilterdb::RandomAccessFile *file = nullptr;
    ASSERT_TAGFILTERDB_OK(env_->NewRandomAccessFile(file_path, &file));
    CheckCloseOnExecDoesNotLeakFDs(open_fds);
    delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecWritableFile) {
    ...
    tagfilterdb::WritableFile *file = nullptr;
    ASSERT_TAGFILTERDB_OK(env_->NewWritableFile(file_path, &file));
    CheckCloseOnExecDoesNotLeakFDs(open_fds);
    delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecAppendableFile) {
    ...
    tagfilterdb::WritableFile *file = nullptr;
    ASSERT_TAGFILTERDB_OK(env_->NewAppendableFile(file_path, &file));
    CheckCloseOnExecDoesNotLeakFDs(open_fds);
    delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecLockFile) {
    ...
    tagfilterdb::FileLock *lock = nullptr;
    ASSERT_TAGFILTERDB_OK(env_->LockFile(file_path, &lock));
    CheckCloseOnExecDoesNotLeakFDs(open_fds);
    ASSERT_TAGFILTERDB_OK(env_->UnlockFile(lock));
...
TEST_F(EnvPosixTest, TestCloseOnExecLogger) {
    ...
    tagfilterdb::Logger *file = nullptr;
    ASSERT_TAGFILTERDB_OK(env_->NewLogger(file_path, &file));
    CheckCloseOnExecDoesNotLeakFDs(open_fds);
    delete file;
...
int main(int argc, char **argv) {
    ...
    tagfilterdb::EnvPosixTest::SetFileLimits(tagfilterdb::kReadOnlyFileLimit,
                                             tagfilterdb::kMMapLimit);
