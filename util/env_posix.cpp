...
class Limiter {
  public:
    ...
    bool Acquire() {
        ...
        // Silence compiler warnings about unused arguments when NDEBUG is
        // defined.
        (void)pre_increment_acquires_allowed;
        // If the check below fails, Release() was called more times than
        // acquire.
...
class PosixMmapReadableFile final : public RandomAccessFile {
  public:
    ...
    // acquired the right to use one mmap region, which will be released when
...
class PosixWritableFile final : public WritableFile {
  ...
  private:
    ...
    //
    // The returned DataView points to |filename|'s data buffer, so it is only
    ...
    Status GetTestDirectory(std::string *result) override {
        ...
        if (env && env[0] != '\0') {
            *result = env;
            ...
            char buf[100];
            std::snprintf(buf, sizeof(buf), "/tmp/tagfilterdbtest-%d",
...
template <typename EnvType> class SingletonEnv {
  public:
    SingletonEnv() {
        ...
        static_assert(offsetof(SingletonEnv<EnvType>, env_storage_) %
                              alignof(EnvType) ==
                          0,
