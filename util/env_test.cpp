...
TEST_F(EnvTest, ReadWrite) {
    ...
    while (data.size() < kDataSize) {
        ...
        ASSERT_TAGFILTERDB_OK(writable_file->Append(r));
        data += r;
        if (rnd.OneIn(10)) {
            ASSERT_TAGFILTERDB_OK(writable_file->Flush());
        }
    }
    ASSERT_TAGFILTERDB_OK(writable_file->Sync());
    ASSERT_TAGFILTERDB_OK(writable_file->Close());
    ...
    while (read_result.size() < data.size()) {
        ...
        DataView read;
        ASSERT_TAGFILTERDB_OK(sequential_file->Read(len, &read, &scratch[0]));
        if (len > 0) {
...
#if defined(TAGFILTERDB_PLATFORM_CHROMIUM)
    // TODO(crbug.com/760362): See comment in MakeIOError() from
    // env_chromium.cc.
    ASSERT_TRUE(status.IsIOError());
#else
    ASSERT_TRUE(status.IsNotFound());
#endif // defined(TAGFILTERDB_PLATFORM_CHROMIUM)
...
TEST_F(EnvTest, ReopenWritableFile) {
    ...
    ASSERT_TAGFILTERDB_OK(writable_file->Append(data));
    ASSERT_TAGFILTERDB_OK(writable_file->Close());
    delete writable_file;
    ...
    ASSERT_TAGFILTERDB_OK(ReadFileToString(env_, test_file_name, &data));
...
TEST_F(EnvTest, ReopenAppendableFile) {
    ...
    std::string data("hello world!");
    ASSERT_TAGFILTERDB_OK(appendable_file->Append(data));
    ASSERT_TAGFILTERDB_OK(appendable_file->Close());
